...
class LetterboxdDiaryRss {
    ...
    parseRss(responseText, count) {
        const dom = htmlparser2.parseDocument(responseText, { xmlMode: true });
        const itemList = htmlparser2.DomUtils.getElementsByTagName("item", dom);
    ...
    getDescriptionDom(item) {
        ...
        return htmlparser2.parseDocument(descriptionHtml, { xmlMode: true });
    }

    getId(item) {
        const results = this.getTextFromTag("guid", item).match(
            /letterboxd-.*-(\d+)/
        );
    ...
    getWatchedDate(item) {
        const watchedDateString = this.getTextFromTag(
    ...
    getImage(item) {
        const elements = htmlparser2.DomUtils.getElementsByTagName("img", item);
        if (!elements.length) {
            return "";
        }
        return htmlparser2.DomUtils.getAttributeValue(elements[0], "src") || "";
    }
    ...
    getReview(item) {
        const paragraphs = htmlparser2.DomUtils.getElementsByTagName("p", item);
        return paragraphs
            .reduce((accumulator, currentValue) => {
                const paragraphText = htmlparser2.DomUtils.getText(
                    currentValue
                ).trim();
    ...
    getContainsSpoilers(item) {
        return this.getTextFromTag("title", item).includes(
            "(contains spoilers)"
        );
    }

    getTextFromTag(tag, item) {
        const elements = htmlparser2.DomUtils.getElementsByTagName(tag, item);
        if (!elements.length) {
            return "";
        }

        return htmlparser2.DomUtils.getText(elements[0]);
    }
}
