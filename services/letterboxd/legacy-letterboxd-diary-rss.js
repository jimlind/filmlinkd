    ...
    getReview(item) {
        return item.childNodes
            .reduce((accumulator, currentValue) => {
                // Get text without paragraph tags or extra whitespace
                const nodeText = this.domSerializer(currentValue)
                    .replace(/<\/*p>/gm, '')
                    .trim();

                // Don't add any autogenerated text that says there are spoilers
                if (nodeText == '<em>This review may contain spoilers.</em>') {
                    return accumulator;
                }
                // Don't add any autogenerated text that says when it was watched
                const watchedRegex = /^Watched on .+day \w+ \d+, \d{4}\.$/;
                if (nodeText.match(watchedRegex)) {
                    return accumulator;
                }
                // Don't add any images
                const imageRegex = /^\<img src=".*"\>$/;
                if (nodeText.match(imageRegex)) {
                    return accumulator;
                }
