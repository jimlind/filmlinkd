export default class LetterboxdLidComparison {
     ...
     * @returns {number}
     */
    compare(letterboxdIdA: any, letterboxdIdB: any) {
        const low = (x: any) => x.toLowerCase();
        const up = (x: any) => x.toUpperCase();
        const swapCase = (x: any) => [...x].map((c) => (c === low(c) ? up(c) : low(c))).join('');
