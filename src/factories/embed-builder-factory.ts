export default class EmbedBuilderFactory {
    ...
    constructor(
        ...
        readonly truncateMarkdown: any,
        readonly turndownService: any,
    ) {}

    createFollowSuccessEmbed(data: any) {
    ...
    createNoAccountFoundEmbed(userName: any) {
    ...
    createUnfollowedSuccessEmbed(data: any) {
        const userName = this.formatUserName(data.userName);
        const description = this.formatDescription(
    ...
    createRefreshErrorEmbed(userName: any) {
        const formattedUserName = this.formatUserName(userName);
     ...
     */
    createFollowingEmbedList(userList: any) {
        const result = [];
        ...
        const lidSorter = (a: any, b: any) =>
            -1 * this.letterboxdLidComparison.compare(a.lid, b.lid);
     ...
     */
    createDiaryEntryEmbed(entry: any, data: any) {
     ...
     * @returns {EmbedBuilder}
     */
    createDiaryListEmbed(member: any, entryList: any) {
        const entryTextList = entryList.map((entry: any) => {
        ...
        const largestImage = member.avatar.sizes.reduce((previous: any, current: any) =>
     ...
     * @returns {EmbedBuilder}
     */
    createFilmEmbed(film: any, filmStatistics: any) {
        let description = '';
        ...
        const filteredDirectorList = film.contributions.filter((c: any) => c.type == 'Director');
        const directorLinkList = (filteredDirectorList[0]?.contributors || []).map(
            (contributor: any) => `[${contributor.name}](https://boxd.it/${contributor.id})`,
        );
        ...
        const countryList = film.countries
        ...
        const genreList = film.genres.map((genre: any) => genre?.name || '').filter(Boolean);
        if (genreList.length) {
     ...
     * @returns {EmbedBuilder}
     */
    createUserEmbed(member: any, memberStatistics: any) {
        let description = '';
        ...
        const filmList = member.favoriteFilms.map(
            (film: any) => `- [${film.name} (${film.releaseYear})](https://boxd.it/${film.id})`,
        );
     ...
     * @returns {EmbedBuilder}
     */
    createListEmbed(listSummary: any) {
     ...
     * @returns {EmbedBuilder}
     */
    createLoggedEmbed(logEntryList: any) {
        if (!logEntryList.length) {
            throw 'Empty List of Log Entry Provided';
        }

        const logEntryTextList = logEntryList.map((logEntry: any) => {
     ...
     * @returns {EmbedBuilder}
     */
    createContributorEmbed(contributor: any) {
        const filmographyString = contributor.statistics.contributions
            .reduce((acc: any, contribution: any) => {
     ...
     * @returns
     */
    createHelpEmbed(config: any, userCount: any, serverCount: any) {
     ...
     */
    formatDate(date: any) {
        // Intent of this code is that we accept a Date object or a number.
        // I don't think we actually send it anything except a Date objec though.
        // I might bother fixing things later.
        ...
        const format: any = Date.now() - date < 5000000000 ? recentFormat : pastFormat;

        const dateObject = new Date(date);
        return dateObject.toLocaleDateString('default', format);
    }
     ...
     * return string
     */
    parseImage(sizes: any) {
        const findLargest = (previous: any, current: any) =>
